...
def main():
    parser = ap.ArgumentParser(description="Generate exams from YAML question bank.")
    parser.add_argument("--bank-files", type=str, nargs='+', default=[], help="Path(s) to the YAML question bank file(s).")
    # parser.add_argument("output_prefix", help="Prefix for output LaTeX exam files.")
    parser.add_argument("--num-versions", type=int, default=1, help="Number of unique exam versions to generate.")
    parser.add_argument("--questions-per-topic", type=int, default=5, help="Number of questions per topic.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for question selection.")
    parser.add_argument("--topics", nargs="+", default=[], help="List of topics to include in the exam (topics are explicitly listed in YAML file).")
    parser.add_argument("--full-dump", action="store_true", help="If set, dump all questions without selection.")
